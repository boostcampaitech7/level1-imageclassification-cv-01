...
class A_aug_test:
    def __init__(self, is_train: bool = True):
        ...
        if is_train:
            ...
            self.transform = A.Compose(
                [
                    A.CLAHE(clip_limit=2.0, tile_grid_size=(8, 8), p=1.0),
                    A.ElasticTransform(alpha=50, sigma=10, border_mode=cv2.BORDER_REFLECT_101),
                    A.GridDistortion(num_steps=5, distort_limit=(-0.3, 0.3), border_mode=cv2.BORDER_REFLECT_101),
                    A.Affine(scale=(0.8, 1.2), translate_percent=(0.1, 0.1), rotate=(-30, 30), shear=(-10, 10),
    ...
    def square_padding(self,image):
        ...
        target_size = max(h, w)
        transform = A.Compose([
            A.PadIfNeeded(min_height=target_size, min_width=target_size, border_mode=cv2.BORDER_CONSTANT, value=[255, 255, 255])  
        ])
